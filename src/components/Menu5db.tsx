...
// EPCC (im-logo-shared-module) - dùng chung logo IM với RtyDashboard.tsx /
// TargetActualDashboard.tsx, xem chi tiết trong imLogo.ts
import { IM_LOGO_DATA_URI, IM_LOGO_ASPECT } from './imLogo';
...
function RevenueTrendChart({
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} width="100%" height={height} preserveAspectRatio="none" role="img">
      ...
      {/* EPCC (menu5-revchart1-prod-label-shift-right) - FIX theo yêu cầu
          người dùng "số hiển thị của sản xuất hãy dịch sang phải": cộng
          thêm offset dương vào `cx` (trục ngang) để nhãn (đang xoay dọc -90°)
          dịch sang phải so với tâm cột, không đổi vị trí theo trục dọc. */}
      {prodVals.map((v, i) => {
        if (v == null) return null;
        const cx = xOf(i) - groupW / 2 + barW + barW / 2 + 5;
        const cy = yBar(v) + 4;
        return (
...
function DonutCenterLogo({ cx, cy, r, theme: _theme }: { cx: number; cy: number; r: number; theme: ThemeMode }) {
  ...
  // nguyên tỉ lệ "to hơn 20%" đã chỉnh trước đó) để logo không bị phóng to
  // thêm ngoài ý muốn khi nền tròn tăng kích thước.
  const logoR = r * 0.86;
  const logoW = logoR * 2 * 0.98 * 1.2;
  const logoH = logoW / IM_LOGO_ASPECT;
  return (
    <g>
      {/* Nền tròn phía sau logo — EPCC (menu5-donut-im-logo-fixed-light-bg)
          theo yêu cầu người dùng "khi chuyển theme không nên chuyển đổi màu
          tại chữ IM và nền background đó, để giống như light mode thôi":
          luôn cố định màu trắng bất kể theme dark/light. */}
...
function RevenueDonutChart({
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} width="100%" height={size} role="img">
      {arcs.map((a, i) => (
        // EPCC (menu5-donut-arc-stroke-fixed-white) - theo yêu cầu người dùng
        // "vòng ring màu đen hãy ... luôn mặc định màu trắng dù đổi theme":
        // trước đây stroke đổi màu theo theme (`#1e293b` ở dark mode) khiến
        // vòng viền quanh tâm logo IM hiện màu đen/tối; nay cố định trắng.
        <path key={i} d={a.d} fill={a.color} stroke="#ffffff" strokeWidth={2} />
      ))}
      ...
      {/* EPCC (menu5-donut-center-close-dark-gap) - theo yêu cầu người dùng
          "vẫn còn đường ring màu đen ... hãy chuyển mặc định để nền trắng":
          khoảng hở đó không phải là nét vẽ mà do vòng tròn trắng (bán kính
          rInner * 0.86) nhỏ hơn mép trong của các lát màu (rInner), để lộ
          nền tối phía sau. Nay cho bán kính vòng tròn trắng khớp đúng
          rInner để lấp kín, không còn khoảng hở. */}
      <DonutCenterLogo cx={cx} cy={cy} r={rInner} theme={theme} />
    </svg>
  );
}
      ...
      <div className={`second-dashboard menu5-dashboard`} style={{ padding: '0 16px 16px 16px', height: '100%', overflowY: 'auto', boxSizing: 'border-box' }}>
        ...
        <style>{`
          .menu5-dashboard .dashboard-header-grid {
            ...
            /* EPCC (menu5-header-card-match-sidebar-header-60px) - đồng bộ
               chiều cao Card đầu tiên (dashboard-header-left) với đúng
               .sidebar-header bên Sidebar (đo Computed panel: height 60px,
               box-sizing border-box, padding 0 16px, align-items center).
               Trước đó height:auto khiến Card tự co theo logo (~50px),
               lệch với Sidebar. Nay ép cứng 60px + border-box để khớp
               chính xác, logo/đồng hồ vẫn giữ margin riêng, chỉ căn giữa
               theo chiều dọc trong khung 60px này. */
        ...
        <div className="dashboard-header-grid">
          {/* EPCC (menu5-header-add-im-logo) - đồng bộ với header chính:
              thêm logo IM, đồng hồ realtime dịch nhẹ sang phải.
              EPCC (menu5-header-logo-no-white-ring) - đồng bộ: bỏ nền elip
              trắng thừa, để lộ đúng màu nền Card; top/bottom quanh logo gần
              như bằng 0.
              EPCC (menu5-header-logo-1mm-vpad-shrink-card) - đồng bộ:
              top/bottom logo = 1mm; Card đầu tiên thấp lại (padding dọc
              10px → 4px).
              EPCC (menu5-header-card-fit-logo-height) - đồng bộ: card tự
              co giãn theo chiều cao logo (height/overflow ép về auto/visible)
              để không bị tràn/cắt.
              EPCC (menu5-header-card-single-source-gap-0x5mm) - đồng bộ: bỏ
              padding dọc của card, chỉ dùng margin logo (0.5mm trên/dưới)
              làm nguồn khoảng cách duy nhất tới mép card.
              EPCC (menu5-header-logo-shrink-15pct) - đồng bộ: logo giảm 15%
              (67.5px → 57.4px), Card tự thấp lại theo. */}
          <div className="dashboard-header-left" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: 'var(--text-2)', fontWeight: 700, whiteSpace: 'nowrap' }}>
            ...
            <span style={{ marginLeft: 6 }}>{formattedTime}</span>
          </div>
    ...
    <div className={`second-dashboard menu5-dashboard`} style={{ padding: '0 16px 16px 16px', height: '100%', overflowY: 'auto', boxSizing: 'border-box' }}>
      ...
      <style>{`
        .menu5-dashboard .dashboard-header-grid {
          ...
          border: 1px solid rgba(0,0,0,0.18);
          box-shadow: 0 2px 8px rgba(0,0,0,0.08);
          /* EPCC (menu5-header-card-match-sidebar-header-60px) - đồng bộ với
             khối CSS ở nhánh return phía trên: ép height 60px + border-box
             để khớp đúng .sidebar-header (Computed: height 60px, border-box,
             padding 0 16px, align-items center) thay vì height:auto co theo
             logo (~50px). */
          height: 60px !important;
          min-height: 60px !important;
          box-sizing: border-box !important;
      ...
      {/* ── Header (copy nguyên văn RtyDashboard.tsx, con trực tiếp của
          outer div — không còn div padding trung gian) ── */}
      <div className="dashboard-header-grid">
        {/* EPCC (menu5-header-add-im-logo) - theo yêu cầu người dùng "vẽ đúng
            logo IM này thêm lên thanh card đầu tiên và realtime dịch sang
            bên phải 1 chút": chèn logo IM (dùng đúng ảnh gốc IM_LOGO_DATA_URI
            đang dùng cho tâm donut) ngay đầu thanh header trái, đồng hồ
            realtime (`formattedTime`) được bọc riêng trong <span> có
            `marginLeft` nhỏ để dịch sang phải so với vị trí cũ.
            EPCC (menu5-header-logo-no-white-ring) - FIX theo yêu cầu người
            dùng "viền màu trắng bên ngoài logo hãy xóa bỏ để giống màu của
            Card đang có, sau đó cân đối lại top/bottom gần như bằng 0": khối
            <span> nền elip trắng thêm ở bước trước tạo ra 1 vòng trắng thừa
            bên ngoài viền oval xanh vốn có sẵn trong chính ảnh logo — nay bỏ
            hẳn <span> đó, chỉ còn <img> trong suốt để lộ đúng màu nền Card
            (xanh đậm) xung quanh logo. `margin: 0, display: 'block'` giữ
            khoảng cách top/bottom quanh logo gần như bằng 0 (không thêm
            padding/margin dọc nào khác trong flex container).
            EPCC (menu5-header-logo-1mm-vpad-shrink-card) - FIX theo yêu cầu
            người dùng "Cho top = 1mm, Bottom = 1mm. Thu nhỏ chiều cao của
            Card đầu tiên lại": margin dọc của logo đổi từ 0 sang đúng 1mm
            trên + 1mm dưới (CSS hỗ trợ trực tiếp đơn vị mm); đồng thời giảm
            padding dọc của `.dashboard-header-grid` (Card đầu tiên) từ
            10px xuống 4px để toàn bộ thanh header thấp lại.
            EPCC (menu5-header-card-fit-logo-height) - FIX theo yêu cầu
            người dùng "kiểm tra lại top=1mm, bottom=1mm so với Card chưa?
            nhìn không đúng mong muốn": logo cao 67.5px lớn hơn chiều cao
            card cố định trước đó (do class `.dashboard-header-grid` gốc có
            thể có height/overflow cố định), khiến logo bị tràn ra ngoài
            card thay vì có khoảng 1mm đều 2 bên bên trong card. Nay ép
            `height: auto`, `min-height: 0`, `overflow: visible`,
            `align-items: center` (đều `!important` để chắc chắn ghi đè) để
            card tự co giãn theo đúng chiều cao logo + padding.
            EPCC (menu5-header-card-single-source-gap-0x5mm) - FIX tiếp theo
            yêu cầu người dùng "top=0.5mm, bottom=0.5mm": trước đó khoảng
            cách bị CHỒNG 2 lớp — padding dọc của card (4px) CỘNG margin dọc
            của logo (1mm) — nên tổng khoảng cách thực tế không khớp con số
            mong muốn. Nay bỏ hẳn padding DỌC của card (`padding: 0 16px`,
            chỉ giữ padding ngang 16px cho 2 bên trái/phải), để margin của
            logo (`0.5mm 0`) là nguồn khoảng cách DUY NHẤT giữa mép card và
            logo — đảm bảo đúng chính xác 0.5mm trên, 0.5mm dưới.
            EPCC (menu5-header-logo-shrink-15pct) - FIX theo yêu cầu người
            dùng "Giảm bớt logo đi 15% và thu nhỏ chiều cao Card thêm nữa":
            logo giảm 15% (67.5px → 67.5 * 0.85 = 57.375 ≈ 57.4px). Vì card
            đã ở chế độ tự co giãn theo đúng chiều cao logo + margin cố định
            0.5mm (không còn padding dọc riêng), nên logo nhỏ lại kéo theo
            Card thấp lại tương ứng, không cần chỉnh gì thêm ở CSS card. */}
        <div className="dashboard-header-left" style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '13px', color: 'var(--text-2)', fontWeight: 700, whiteSpace: 'nowrap' }}>
          <img src={IM_LOGO_DATA_URI} alt="IM" style={{ height: 57.4, width: 'auto', display: 'block', margin: '0.5mm 0' }} />
          <span aria-hidden="true">🕐</span>
          <span style={{ marginLeft: 6 }}>{formattedTime}</span>
