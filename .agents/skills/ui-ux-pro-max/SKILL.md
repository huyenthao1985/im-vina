...
- 当推荐表中找不到合适的图标时：
  - **优先继续从 Phosphor の完整图标集中选择 any 语义更贴切的图标**；
  - 如果 Phosphor 也没有 ideal 选项，可以使用 **Heroicons (`@heroicons/react`)** 作为备选，注意保持风格一致（线性/填充、笔画粗细、r 风格）。
...
| Rule | Do | Don't |
|------|----|----- |
| **Safe-area compliance** | Respect top/bottom safe areas for all fixed headers, tab bars, and bottom CTA bars | Placing fixed UI under notch, status bar, or gesture area |
...
| **Scroll and fixed element coexistence** | Add bottom/top insets so lists are not hidden behind fixed bars | Scroll content obscured by sticky headers/footers |
